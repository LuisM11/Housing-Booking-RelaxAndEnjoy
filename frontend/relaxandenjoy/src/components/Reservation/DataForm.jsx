...
function DataForm({ product }) {
  const { getCitiesList, user, setUser, getProductReservations } =
    useGlobalContext();
  ...
  const onSubmit = (e) => {
    ...
    if (dates.length == 0 || arrival == "") {
      Swal.fire({
        ...
        text:
          "Debes especificar " +
          (dates.length == 0 ? "las fechas de reserva" : "") +
          (dates.length == 0 && arrival == "" ? " y " : "") +
          (arrival == "" ? "la hora de llegada" : ""),
  ...
  const bookFetch = async () => {
    ...
    const response = await axios
      .post("http://3.139.250.124:8080/reservations", dataFetch, {
  ...
  const disabledDate = (current) => {
    ...
    currentReservations?.every((x) => {
      ...
      bool =
        current > moment(x.initDate) &&
        (current < moment(x.finaltDate) ||
  ...
  return (
    ...
    >
      <h2
        onClick={() => console.log(currentReservations)}
        className="text-secundaryColor text-xl font-bold p-3"
      >
        Completá tus datos
      </h2>
      <div className="grid grid-cols-1 desktop:grid-cols-3 desktop:row-span-3 gap-8 px-3 mb-8">
        ...
        <article className="grid desktop:col-start-1 desktop:col-span-2">
          ...
          <Calendar
            ...
            className="tablet:hidden w-full rounded-lg shadow-md border-none text-thirdColor bg-white brdrs p-10"
          />
          <Calendar
            tileDisabled={disableCalendar}
            showDoubleView
